...
namespace Cours_1
{
    public partial class EcranHorloge : Form
    {
        ...
        private void EcranHorloge_Paint(object sender, PaintEventArgs e)
        {
            ...
            x = xcentre + (int)(0.9 * ax * Math.Cos(heure.Second * Math.PI / 30 - Math.PI / 2));
            y = ycentre + (int)(0.9 * ay * Math.Sin(heure.Second * Math.PI / 30 - Math.PI / 2));
            e.Graphics.DrawLine(new Pen(Color.Black), xcentre, ycentre, x, y);
            //affiche l'aiguille des minutes
            x = xcentre + (int)(0.8 * ax * Math.Cos(heure.Minute * Math.PI / 30 - Math.PI / 2));
            y = ycentre + (int)(0.8 * ay * Math.Sin(heure.Minute * Math.PI / 30 - Math.PI / 2));
